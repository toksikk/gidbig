...
const (
	...
	violationWindow    = 14 * 24 * time.Hour
	orderSweepInterval = time.Minute
)

var (
	banDurations        = [...]time.Duration{0, 6 * time.Hour, 12 * time.Hour, 2 * 24 * time.Hour}
	probationDurations  = [...]time.Duration{0, 12 * time.Hour, 1 * 24 * time.Hour, 2 * 24 * time.Hour}
