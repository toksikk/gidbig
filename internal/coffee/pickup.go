...
const (
	...
	violationWindow    = 7 * 24 * time.Hour
	orderSweepInterval = time.Minute
)

var (
	banDurations        = [...]time.Duration{0, 3 * time.Hour, 6 * time.Hour, 1 * 24 * time.Hour}
	probationDurations  = [...]time.Duration{0, 6 * time.Hour, 12 * time.Hour, 1 * 24 * time.Hour}
