...
func TestPickupViolationsEscalateGlobally(t *testing.T) {
	...
	if stageOne.Stage != 1 || !stageOne.BlockedUntil.Equal(start.Add(3*time.Hour+3*time.Hour)) || !stageOne.ProbationUntil.Equal(stageOne.BlockedUntil.Add(6*time.Hour)) {
	...
	if stageTwo.Stage != 2 || !stageTwo.BlockedUntil.Equal(afterFirstBan.Add(time.Hour+6*time.Hour)) || !stageTwo.ProbationUntil.Equal(stageTwo.BlockedUntil.Add(12*time.Hour)) {
	...
	if stageThree.Stage != 3 || !stageThree.BlockedUntil.Equal(afterSecondBan.Add(24*time.Hour)) || !stageThree.ProbationUntil.Equal(stageThree.BlockedUntil.Add(24*time.Hour)) {
...
func TestOldViolationsDoNotTriggerFirstBan(t *testing.T) {
	...
	if count != 0 {
		t.Fatal("a violation older than 7 days must not trigger a ban")
	}
}

func TestAugustAndSeptemberMissesDoNotTriggerBan(t *testing.T) {
	m := newTestModule(t)
	for id, at := range []time.Time{
		time.Date(2026, 8, 21, 12, 0, 0, 0, time.UTC),
		time.Date(2026, 8, 31, 12, 0, 0, 0, time.UTC),
		time.Date(2026, 9, 23, 12, 0, 0, 0, time.UTC),
	} {
		recordViolation(t, m, uint(id+1), "u1", at)
	}
	var restrictions int64
	if err := m.getDB().Model(&BrewRestriction{}).Where("user_id = ?", "u1").Count(&restrictions).Error; err != nil {
		t.Fatalf("count restrictions: %v", err)
	}
	if restrictions != 0 {
		t.Fatalf("restrictions = %d, want 0", restrictions)
	}
}
...
func TestPickupPenaltyStatsShowsStrikesTimeoutsAndProbation(t *testing.T) {
	...
	for id := uint(4); id <= 6; id++ {
		recordViolation(t, m, id, "probation", now.Add(-6*time.Hour+time.Duration(id-4)*time.Hour))
	}
