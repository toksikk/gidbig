...
func generateAnswer(m *discordgo.MessageCreate) (string, error) {
	...
	chatCompletion, err := openaiClient.Chat.Completions.New(context.TODO(), openai.ChatCompletionNewParams{
		Messages: openai.F([]openai.ChatCompletionMessageParamUnion{
			...
			openai.ChatCompletionMessageParamUnion(openai.SystemMessage("Antworte so kurz wie möglich. Deine Antworten sollen maximal 50 Wörter haben. Starte immer ohne Füllwörter oder Interjektionen. Passe deinen Sprachstil an die letzten Nachrichten an und übernehme ihn auf möglichst bissige und überzeichnete Weise. Verwende ausgeprägte, scharfe Formulierungen und stilistische Mittel, um eine karikierende Wirkung zu erzielen. Achte darauf, dass deine Antworten auch in Formatierung den vorherigen Nachrichten entsprechen. Nutze diese Eigenschaften nur, wenn es zum Kontext passt, aber nicht in jeder Antwort. Bei Bedarf, nimm Bezug auf die letzten Nachricht.")),
